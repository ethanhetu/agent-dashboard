...
def agent_dashboard():
    ...
    for idx, (_, player) in enumerate(top_clients.iterrows()):
        with client_cols[idx]:
            ...
            if img_path:
                st.image(img_path, width=200, output_format="PNG", caption=player['Combined Names'], use_column_width=False)
            else:
            ...
            st.markdown("""
                <div style='border: 1px solid #dcdcdc; border-radius: 10px; padding: 10px;'>
                    <p><strong>Age:</strong> {age}</p>
                    <p><strong>Six-Year Agent Delivery:</strong> {delivery}</p>
                    <p><strong>Six-Year Player Cost:</strong> ${cost:,}</p>
                    <p><strong>Six-Year Player Contribution:</strong> ${contribution:,}</p>
                </div>
                <div style='text-align:center; font-weight:bold; font-size:18px;'>
                    Value Capture Percentage: {percentage:.2%}
                </div>
                """.format(
                ...
                percentage=player['Value Capture %']
