...
def load_data():
    ...
    agency_ranks_data = xls.parse('Agency Ranks')
    piba_data = xls.parse('PIBA')
    piba_data.columns = piba_data.columns.str.strip()  # Remove extra spaces in column names
    return agents_data, agencies_data, ranks_data, agency_ranks_data, piba_data
...
def display_financial_overview(group_info, rank_info=None, total_entities=None):
    ...
    if rank_info is not None and total_entities is not None:
        col5.metric("Ranking", f"#{int(rank_info)}/{total_entities}")

def dashboard_template(group_name, group_by_column, summary_data, rank_data, piba_data, total_entities):
    ...
    rank_info = rank_data[rank_data[group_by_column] == selected_group].iloc[0]['Rank'] if not rank_data[rank_data[group_by_column] == selected_group].empty else None
    display_financial_overview(group_info, rank_info, total_entities)
...
def agent_dashboard():
    agents_data, agencies_data, ranks_data, agency_ranks_data, piba_data = load_data()
    extract_headshots()
    dashboard_template("Agent", "Agent Name", agents_data, ranks_data, piba_data, 90)

def agency_dashboard():
    agents_data, agencies_data, ranks_data, agency_ranks_data, piba_data = load_data()
    extract_headshots()
    dashboard_template("Agency", "Agency Name", agencies_data, agency_ranks_data, piba_data, 74)
